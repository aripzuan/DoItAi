  ...
  return (
    <div className='h-full overflow-y-scroll p-1 flex items-start flex-wrap gap-2 text-slate-700'>
        ...
        <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-2 bg-white rounded-lg border border-gray-200'>
          ...
          <input onChange={(e)=>setInput(e.target.files[0])} type="file" accept='image/*' className='w-full p-2 px-3 mt-1 outline-none text-sm rounded-md border border-gray-300 text-gray-600' required />

          <p className='mt-2 text-sm font-medium'>Describe object name to remove</p>
          <textarea onChange={(e)=>setObject(e.target.value)} value={object} rows={2} className='w-full p-2 px-3 mt-1 outline-none text-sm rounded-md border border-gray-300'placeholder='e.g, tree or car, Only one single object can be removed...' required />

          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 mt-2 text-sm rounded-lg cursor-pointer'>
        ...
        <div className='w-full max-w-lg p-2 bg-white rounded-lg flex flex-col border border-gray-200 min-h-64'>
            ...
            :
             (
              <img src={content} alt="image" className='mt-2 w-full h-full' />
             )
            }
