  ...
  return (
       <div className='h-full overflow-y-scroll p-1 flex items-start flex-wrap gap-2 text-slate-700'>
        ...
        <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-2 bg-white rounded-lg border border-gray-200'>
          ...
          <p className='mt-2 text-sm font-medium'>Upload Resume</p>

          <input onChange={(e)=>setInput(e.target.files[0])} type="file" accept='application/pdf' className='w-full p-2 px-3 mt-1 outline-none text-sm rounded-md border border-gray-300 text-gray-600' required />

          <p className='text-xs text-gray-500 font-light mt-1'>Supports PDF only.</p>

          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00DA83] to-[#009BB3] text-white px-4 py-2 mt-2 text-sm rounded-lg cursor-pointer'>
